...
export default function EventFormPage() {
    ...
    if (!id) {
      ...
      const fetchEventData = async () => {
        ...
        const { data, error } = await supabase
          .from('events')
          .select('title, description, deadline, category, form_schema, organizer_id, start, end, images, venue_name, venue_address, share_link')
          .eq('event_id', id)
          .single()
  ...
  const addQuestion = () => {
    setCustomQuestions(prev => [
      ...prev,
      { id: uuidv4(), label: '新問題', type: 'text', required: false, options: [] },
    ])
  }
  ...
  if (notAuthorized) return <p className="p-4 text-red-600 dark:text-red-400">
    您沒有權限查看此報名資料。
  </p>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 text-gray-900 dark:text-white">
      ...
      <div className="flex justify-center mb-6 space-x-4">
        <button
          className={`px-4 py-2 font-semibold rounded ${activeTab === 'basic'
        ...
        <button
          className={`px-4 py-2 font-semibold rounded ${activeTab === 'form'
            ? 'bg-blue-600 text-white'
            : 'bg-gray-200 dark:bg-gray-700 dark:text-gray-300'}`}
      ...
      {activeTab === 'basic' && (
        <>
          ...
          {timeError && <p className="text-red-600 text-sm mb-4">{timeError}</p>}
          {/* Venue */}
          <div className="flex items-center gap-4 w-full">
            <label className="flex-1">
              活動地點名稱
                      ...
                      <span key={i} className="bg-blue-100 dark:bg-blue-800 px-2 py-1 rounded text-sm flex items-center">
                        ...
                        <button
                          ...
                          onClick={() => updateQuestion(q.id, {
                            options: q.options.filter((_: any, idx: number) => idx !== i),
                          })}
                        >✕</button>
                      </span>
                    ))}
