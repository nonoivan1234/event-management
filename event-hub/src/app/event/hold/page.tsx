  ...
  useEffect(() => {
    const fetchUserAndEvents = async () => {
      ...
      const { data: organizedData } = await supabase
        ...
        .select(
          "events(event_id, title, description, created_at, visible, deadline)"
        )
        .eq("role_id", user.id)
        .eq("role", "organizer");
            ...
            <section className="mb-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredEvents(organizedEvents).map((event) => (
                  <div
                    key={event.event_id}
                    className="border rounded-xl p-5 shadow-sm bg-white dark:bg-gray-800 dark:border-gray-700 transition-all duration-300 hover:shadow-lg hover:scale-[1.01]"
                  >
